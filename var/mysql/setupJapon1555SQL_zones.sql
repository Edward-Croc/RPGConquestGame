...
-- Secrets scénario
INSERT INTO {prefix}locations (name, discovery_diff, zone_id, controller_id, description, hidden_description) Values
    -- Ajouter un secret sur l'arrivée des rebels Ikko-ikki sur l'ile par petits groupes
    ('Plaine d’Uwajima', 7, (SELECT ID FROM {prefix}zones WHERE name = 'Côte Ouest d’Iyo'), (SELECT ID FROM {prefix}controllers WHERE lastname = 'Jōdo-shinshū (浄土真宗)')
...
Si vous avez entendu parler d’une rumeur sortant de la maison close de Marugame, ce n’est qu’une version déformée de cette vérité.',
    ...
    , ('Sanctuaire des blessés de guerre', 8, (SELECT ID FROM {prefix}zones WHERE name = 'Ile d’Awaji'), NULL,
...
(Vous pouvez demander à l’orga la carte 3 et 4 de la bataille)'
    ...
    , ('Camp de deserteurs', 7, (SELECT ID FROM {prefix}zones WHERE name = 'Ile de Shōdoshima'), NULL
...
(Vous pouvez demander les cartes 1 et 2 de la bataille.)'
    ...
    , ('Cour impériale', 7, (SELECT ID FROM {prefix}zones WHERE name = 'Cité impériale de Kyōto'), NULL
...
-- Warning: If you read this file, you will no longer be eligible to participate as a player.
-- Secret des joueurs
INSERT INTO {prefix}locations (name, description, hidden_description, discovery_diff, can_be_destroyed, zone_id, controller_id) VALUES
...
Peu en ressortent, et ceux qui le font ne parlent plus.',
        8, 1,
        (SELECT ID FROM {prefix}zones WHERE name = 'Cité impériale de Kyōto'),
        (SELECT ID FROM {prefix}controllers WHERE lastname = 'Ashikaga (足利)'),
        '{"indestructible":"TRUE"}'
    ),
...
Ce lieu sert également de relais discret pour faire entrer armes, livres et messagers depuis Nagasaki.'
        ...
        (SELECT ID FROM {prefix}controllers WHERE lastname = 'Sogō (十河)'),
        '{"update_location": {
...
(Pour explorer davantage ce lieu, allez voir un orga !)',
    ...
    -- Vent - Tessen
    , ('Vieux temple de la falaise d’Esaki', 8, 1, (SELECT ID FROM {prefix}zones WHERE name = 'Ile d’Awaji'),  (SELECT ID FROM {prefix}controllers WHERE lastname = 'Shikoku (四国)'),
...
(Pour explorer davantage ce lieu, allez voir un orga !)',
     ...
     -- Paresse - Biwa
    , ('Vieux temple du vallon de Tengu-Iwa', 7, 1, (SELECT ID FROM {prefix}zones WHERE name = 'Ile de Shōdoshima'),  (SELECT ID FROM {prefix}controllers WHERE lastname = 'Shikoku (四国)'),
...
(Pour explorer davantage ce lieu, allez voir un orga !)',
     ...
     -- Roche - Chigiriki
    , ('Vieux temple du Mont Ishizuchi', 8, 1, (SELECT ID FROM {prefix}zones WHERE name = 'Montagnes d’Iyo'),  (SELECT ID FROM {prefix}controllers WHERE lastname = 'Shikoku (四国)'),
...
Certains racontent avoir vu un noble personnage tenter de rentrer en secret avant d’être intercepté par les Kaizokushū.',
    ...
    ('Suzaku Mon',
     ...
     6, (SELECT ID FROM {prefix}zones WHERE name = 'Cité impériale de Kyōto'))
;
...
Depuis quelques jours, un bruit court : un important émissaire impérial aurait été intercepté par les pirates Wako et détenu dans une grotte voisine, en attendant rançon ou silence.'
    ...
    ('Maison de thé de la Lune d’Or', 5, (SELECT ID FROM {prefix}zones WHERE name = 'Cité impériale de Kyōto'),
...
(Si vous voulez entrer en contact avec les Takeda, allez voir un orga ! )',
    5, 1, 1, 0, (SELECT ID FROM {prefix}zones WHERE name = 'Plaines du Kansai'),
...
-- Warning: If you read this file, you will no longer be eligible to participate as a player.
INSERT INTO {prefix}locations (name, description, hidden_description, zone_id, controller_id, is_base, can_be_destroyed, can_be_repaired, discovery_diff, activate_json) VALUES
(
...
INSERT INTO {prefix}artefacts (name, description, full_description, location_id) VALUES
    (
        ...
        'Nous sommes libres de décider de sa destinée (aller voir un orga)!', (SELECT ID FROM {prefix}locations WHERE name = 'Sanctuaire clandestin du Port de Tokushima')
    )
