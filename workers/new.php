...
if (isset($_GET['recrutement'])) {
    ...
    if (!((int)$controllerValues[0]['turn_firstcome_workers'] < (int)getConfig($gameReady, 'turn_firstcome_workers'))) {
        $recrutment_allowed = false;
    }
}
...
for ($iteration = 0; $iteration < $nbChoices; $iteration++) {
    ...
    if ($_SESSION['DEBUG_TRANSFORM']) {
        echo sprintf("recrutement_transformation_array :%s  <br>", var_export($recrutement_transformation_array, true));
    }
