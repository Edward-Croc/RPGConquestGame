...
if (isset($_SESSION['controller'])) {
    ...
    $htmlFaction = sprintf(
        '<h2 class="title is-4">Votre Faction</h2>
                <div class="box mb-4">
                <p>
    ...
    if (!empty($incomingAttacks)) {
        ...
        foreach ($byTurn as $turn => $atks) {
            ...
            $items = '';
            foreach ($atks as $atk) {
                $items .= sprintf('<li>%s</li>', $atk['target_result_text']);
            }
            $panels .= sprintf(
    ...
    $byTurn = [];
    foreach ($outgoingAttacks as $atk) {
        $byTurn[(int)$atk['turn']][] = $atk['attacker_result_text'];
    }
    ...
    if (empty($receivedInfoGifts)) {
        ...
        foreach ($receivedInfoGifts as $gift) {
            $giftsByTurn[(int)$gift['turn']][] = $gift;
        }
        krsort($giftsByTurn);
        $tabs = '<div class="tabs title"><ul>';
        $panels = '';
        ...
        foreach ($giftsByTurn as $turn => $turnGifts) {
            ...
            foreach ($turnGifts as $gift) {
                ...
                $items .= '<li>' . sprintf(
                    ...
                    htmlspecialchars($gift['zone_name'] ?? '') ?: '—'
                ) . '</li>';
            }
            $panels .= sprintf(
