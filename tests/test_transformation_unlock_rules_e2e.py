...
# ---------------------------------------------------------------------------
# R10 — Vampire1966 Possession OR regression
# ---------------------------------------------------------------------------

class TestVampire1966PossessionOR:
    """Regression check for R10: the Possession row's pre-fix OR was a
    single object that PHP iterated as scalar keys → $OR stayed false →
    power fully hidden. Post-fix it is an array of two single-key
    branches matching D12 (array-of-objects) + D20 (one rule per branch).

    Belt-and-buckle DB check: read the Possession row, decode its
    on_transformation.OR, assert it is a list of 2 single-key dicts."""

    @pytest.fixture(scope="class", autouse=True)
    def vampire_scenario(self, browser):
    ...
    def test_possession_or_shape_is_array_of_single_key_objects(self):
        ...
        assert isinstance(or_block, list), (
            f"D12: OR must be array-of-objects, got {type(or_block).__name__}: {or_block}"
        )
        assert len(or_block) == 2, (
            f"Possession fix locks OR as TWO single-key branches; got {or_block}"
        )
        for branch in or_block:
            assert isinstance(branch, dict), (
                f"D20: each OR branch must be an object; got {type(branch).__name__}"
            )
            assert len(branch) == 1, (
                f"R10 fix authoring convention: one key per branch; got {branch}"
            )
