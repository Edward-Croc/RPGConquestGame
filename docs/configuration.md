...
faces d'un transfert de garde (`transferPrisoner`). La première est déposée sur
la trace que le geôlier conserve, la seconde sur le rapport de l'agent lui-même,
chez son nouveau geôlier. Deux paramètres positionnels chacune : `%1$s` = le
dénominateur `controllerNameDenominatorThe`, `%2$s` = le nom de la faction
receveuse. Un transfert refusé n'écrit aucune des deux.
